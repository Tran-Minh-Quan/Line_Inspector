...
class Line:
    ...
    def detect_error(self,template_dir, min_thres = 36, max_thres = 48):
        ...
        #edge_image = cv2.Canny(opening, 50, 200, apertureSize=3)
    ...
    while(True):
        ...
        if not ok:
          ...
          if line.status != 'Error':
            cv2.putText(frame, line.status, (line.top_left_x-130,line.top_left_y+20), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,255,0),2, lineType=cv2.LINE_AA)
            cv2.rectangle(frame, (line.top_left_x, line.top_left_y), (line.bottom_right_x, line.bottom_right_y), (0,255,0), 2)
          else:
            cv2.putText(frame, line.status, (line.top_left_x-130,line.top_left_y+20), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,0,255),2, lineType=cv2.LINE_AA)
            cv2.rectangle(frame, (line.top_left_x, line.top_left_y), (line.bottom_right_x, line.bottom_right_y), (0,0,255), 2)
            N_bad.append(line.status=='Error')
