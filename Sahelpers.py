...
def extract_image_from_video(videolink='E:/WON/DO_AN/Training/not_cut_video/VIDEO_capture_1.avi',saveFolderLink ='E:/WON/DO_AN/Training/cut_video/',beginNum = 240,name = '1_damper_', imtype= '.jpg'):
  ...
  for i in range(frameNum):
    ...
    if k == 1:
      cv2.imwrite(saveFolderLink + name + str(beginNum+imNum) + imtype, frame)
      print(str(imNum+1)+'/'+str(frameNum))
      imNum = imNum +1
      k = 0
    else: k = k+1
...
def folder_rename(rename_folder_path = 'D:/WON/DO_AN/Code/Code_Sa/test/ball/Image/',extension = '.jpg',new_name = '0_ta_',begin_index = 1):
  number_string = ''
  folder_list = os.listdir(rename_folder_path)
  for filename in folder_list:
    for character in filename:
      if  character.isnumeric() == True:
        number_string = number_string + character
    src = rename_folder_path + filename
    dst = rename_folder_path + number_string
    os.rename(src, dst)
    #time.sleep(0.001)
    number_string = ''
  folder_list = sorted(os.listdir(rename_folder_path),key=int)
  #print(folder_list)

  for count, filename in enumerate(folder_list,begin_index):
    print(str(count)+'/'+str(len(folder_list)+begin_index-1))
    #print(filename)
    src = rename_folder_path + filename
    dst = rename_folder_path + new_name + str(count) + extension
    os.rename(src,dst)
    #time.sleep(0.001)
...
def pyramid(image, scale=1, minSize=(30, 30)):
  while True:
    ...
    if image.shape[0] < minSize[1] or image.shape[1] < minSize[0]:
      break
    return image
def CaptureSample(imglink='',dlink='',name='image',imtype='.jpg',cam=0,heigth=480,width=640,color=1,timesleep=0.05,i=1, dmax = 60):
    ...
    while(True):
        ...
        if (k == 27):
            ...
            cv2.destroyAllWindows()
            cv2.imwrite(imglink+name+str(i)+imtype,frame)
            cv2.imshow(name+str(i)+imtype+'  '+str(dmax)+'cm'+'  saved',frame)
            ...
            cv2.destroyAllWindows()
            if (k == ord('d')):
              os.remove(imglink + name + str(i) + imtype)
              os.remove(dlink+name+str(i)+'.txt')
              cv2.imshow(name + str(i) + imtype +'  '+str(dmax)+'cm'+'  deleted', frame)
              cv2.waitKey()
              cv2.destroyAllWindows()
            else:
              i += 1
...
#tự động lưu lại các frame từ camera cách nhau captureTime
#sau khi thu được số frame = imNum thì tự động thoát ra
#copy link thư mục lưu ảnh vào link=''
def AutoCaptureSample(link='', name='',captureTime=1/30,imNum=500,imtype='.png', cam=0, heigth=480, width=640, color=1, i=1):
    cap=cv2.VideoCapture(cam)
    cap.set(4,int(heigth) )
    cap.set(3,int(width))
    if not cap.isOpened():
