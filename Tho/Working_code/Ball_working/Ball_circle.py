...
img = cv2.imread("Ball/ball_56_1.jpg")
red_channel = img[:, :, 2]
green_channel = img[:, :, 1]
blue_channel = img[:, :, 0]
# cv2.namedWindow("Eval image")
cv2.namedWindow("Result")
cv2.createTrackbar("Green_coefficient", "Result", 500, 4000, nothing)  # 112 #500
cv2.createTrackbar("Blue_coefficient", "Result", 1038, 4000, nothing)  # 3300 #1038
kernel = np.ones((5, 5), np.uint8)
while True:
    green_coefficient = cv2.getTrackbarPos("Green_coefficient", "Result") / 1000
    blue_coefficient = cv2.getTrackbarPos("Blue_coefficient", "Result") / 1000
